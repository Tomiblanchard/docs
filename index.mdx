title: "Primality Documentation"
description: "SwiftUI + RealityKit capture companion for high quality occlusion shots on LiDAR-enabled iOS devices."
---

## Overview

Primality is a SwiftUI + RealityKit capture companion for collecting high-quality occlusion shots on LiDAR-enabled iOS devices. It guides operators through a scripted flow, records synchronized video, audio, depth, and AR session metadata, and packages each capture into a reproducible scene bundle with automated QC metrics.
...
1. Clone the repository and open `Primality.xcodeproj` (or the workspace described in `buildServer.json`).
...
- **Auto-save (optional):** When a Files destination is chosen, scene folders are mirrored there; otherwise they are kept under **On My iPhone/iPad > Primality/AutoSaves**.
...
To fetch bundles without the document picker, connect the device in Finder and browse **Files > Primality > scenes**.
...
- `Primality/ContentView.swift` — SwiftUI composition for the capture UI, overlays, guided flow, and toolbar affordances.
- `Primality/CapturePipeline.swift` — Core AR session pipeline handling state management, recording, QC, manifest synthesis, and gating logic.
- `Primality/DepthWriter.swift`, `VideoWriter.swift`, `AudioRecorder.swift` — Modality-specific writers.
- `Primality/FramesJSONL.swift`, `Manifest.swift`, `Checksum.swift` — Serialization helpers for metadata exports.
- `Primality/ShotDashboard.swift`, `ShotModels.swift`, `SceneRecipe.swift` — Shot selection UI, JSON schema models, and prompt derivation.
- `Primality/MeshExporter.swift` — OBJ mesh export from `ARMeshAnchors` (USDZ placeholder maintained for future work).
- `Primality/AutoSaveManager.swift`, `DocumentExporter.swift`, `Uploader.swift` — Post-capture tooling.
- `PrimalityTests/`, `PrimalityUITests/` — Swift Testing and XCTest scaffolding (placeholders ready for expansion).
...
```bash
xcodebuild test \
  -workspace Primality.xcodeproj/project.xcworkspace \
  -scheme Primality \
  -destination 'platform=iOS Simulator,name=iPhone 15'
```

Simulator tests cover logic only; capture features require a real device. UI tests under `PrimalityUITests` launch the app shell and include a launch performance baseline.
